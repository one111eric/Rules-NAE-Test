package impl;

/**
 * Properties used by NAE test
 *
 */
public class NAE_Properties {
	public static final String NAE_URL = "http://nae.qa.rules.comcast.com/notify";
	public static final String MOCK_SERVER="mock.rules.vacsv.com";
	public static final int MOCK_SERVER_PORT=8080;
	public static final boolean WITH_X_DEBUG_HEADER = true;
	public static final boolean WITHOUT_X_DEBUG_HEADER = false;
	public static final String MOCKSERVER_ADDRESS = "http://mock.rules.vacsv.com:8080";
	public static final String EEL_ENDPOINT="http://eel.qa.rules.comcast.com/";
	public static final String REST_ENDPOINT="http://rest.qa.rules.comcast.com/";
	public static final String PROVISION_ENDPOINT="http://rest.qa.rules.comcast.com/provisions";
	public static final String EEL_EVENT_ENDPOINT="http://eel.qa.rules.comcast.com/elementsevent";
	
}
