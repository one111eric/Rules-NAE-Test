...
Scenario Outline: Basic end to end to verify new rule works 
	...
	When I check the number of request from "<location>" received by mock server 1 
	Then I should get a number 1
	And I post an Event of "<location>" to EEL with SessionId "<SessionId>" 
	Then I should see the number of the request to that location increased by <x> 1
	
	Examples: 
		|location|ttl_time|SessionId|x|
		|MiaoLocation00005|10|miaoSession1|1|
